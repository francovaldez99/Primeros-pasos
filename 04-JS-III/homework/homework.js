...
function incrementarPorUno(array) {
  ...
  // Tu código:
var contador=0
for(contador;contador<=(array.length-1);contador++){
  array[contador]++
}return array
}
...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  var cadenas=palabras[0]
  for(var i=1;i<=(palabras.length-1);i++){
    cadenas=cadenas+" "+palabras[i]
  }return cadenas
}
...
function arrayContiene(array, elemento) {
  ...
  for(i=0;i<=(array.length-1);i++){
    valor=true;
    if(array[i]===elemento)return valor
    else valor=false 
  }return valor
...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  else if(arguments.length==0) return arguments[0]
  var contador=1;
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  var comprobar=arreglo[0]
  var condicion=null
  for(var i=0;i<arreglo.length;i++){
    if(comprobar===arreglo[i]) condicion=true
